        ...
        def process_chunk(chunk):
            ...
            for i in range(batch_size):
                ...
                if mode == "color_key":
                    ...
                    # 创建 mask
                    mask = (color_dist > threshold).to(frame.dtype)
                    ...
                    mask = (brightness > threshold).to(frame.dtype)
                    ...
                    mask = (edge > threshold).to(frame.dtype)
                    ...
                    result = frame
                    mask = frame.new_ones(frame.shape[0], frame.shape[1])
...
class BackgroundReplace:
    ...
    def execute(self, foreground: torch.Tensor, background: torch.Tensor,
        ...
        if mask is None:
            ...
            mask = (brightness > 0.05).to(fg.dtype)
        ...
        # 确保 mask 维度正确
        mask = mask[:min_frames].to(device=fg.device, dtype=fg.dtype)
    ...
    def execute(self, images: torch.Tensor, key_color: str,
        ...
        def process_chunk(chunk):
            ...
            for i in range(chunk.shape[0]):
                ...
                mask = (color_dist > tolerance).to(frame.dtype)

                # 边缘柔和化
                if softness > 0:
                    # 计算软边缘
                    soft_threshold = tolerance + softness
                    hard_mask = (color_dist > soft_threshold).to(frame.dtype)
                    soft_mask = (color_dist > tolerance).to(frame.dtype)
...
class SimpleBackgroundRemove:
    ...
    def execute(self, images: torch.Tensor, bg_color: str, tolerance: float):
        ...
        bg_color_tensor = images.new_tensor([r, g, b])
        ...
        def process_chunk(chunk):
            ...
            for i in range(chunk.shape[0]):
                ...
                # 计算与背景颜色的差异（在输入设备上）
                diff = torch.abs(frame - bg_color_tensor)
                max_diff = diff.max(dim=-1)[0]

                # 创建 mask
                mask = (max_diff > tolerance).to(frame.dtype)
