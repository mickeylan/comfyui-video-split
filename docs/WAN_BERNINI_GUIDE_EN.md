...
The first segment uses the original I2V task. The node decodes its final frame, creates a fresh native `WanImageToVideo` task for the next segment, removes duplicate boundary frames, and assembles CPU images.
